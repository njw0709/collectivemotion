...
for tstep=0:maxt
    ...
    for q = 1:length(xvel)
        ...
        dircosqpt = yvelqpt./magqpt;
            ...
            avcorr(tstep+1,1) = avcorr(tstep+1,1)+m*n; %adds to the counter for the number of pairs at this tstep
            avcorr(tstep+1,2) = avcorr(tstep+1,2)+sum(sum(magq.*magqpt)); %increments running tally of sum(v_i*v_j) 
            avcorr(tstep+1,3) = avcorr(tstep+1,3)+sum(sum(dircosq.*dircosqpt));  %increments running tally of sum(dcos_i*dcos_j)
...
%  
    ...
    ending = floor(length(magangtimecorr(:,1))*9/10);
    [xdata1, ydata1] = prepareCurveData(magangtimecorr(:,1),magangtimecorr(:,2));
    [xdata2, ydata2] = prepareCurveData(magangtimecorr(:,1),magangtimecorr(:,3));

    cutdata1 = excludedata(xdata1,ydata1,'box',[magangtimecorr(3,1) magangtimecorr(ending,1) 0 1]);
    cutdata2 = excludedata(xdata2,ydata2,'box',[magangtimecorr(3,1) magangtimecorr(ending,1) 0 1]);
    % Set up fittype and options.
    ft = fittype( 'a*exp(-b*x)', 'independent', 'x', 'dependent', 'y');
    
    a_start_mag=magangtimecorr(3,2);
    opts1 = fitoptions( 'Method', 'NonlinearLeastSquares','Exclude',cutdata1);
    opts1.Display = 'Off';
    opts1.Robust = 'Bisquare';
    opts1.StartPoint = [a_start_mag,0.3];
    opts1.upper = [1,10];
    opts1.lower = [0,0];
    
    a_start_ang=magangtimecorr(3,3);
    opts2 = fitoptions( 'Method', 'NonlinearLeastSquares','Exclude',cutdata2);
    opts2.Display = 'Off';
    opts2.Robust = 'Bisquare';
    opts2.StartPoint = [a_start_ang, 0.3];
    opts2.upper = [1,10];
    opts2.lower = [0,0];
