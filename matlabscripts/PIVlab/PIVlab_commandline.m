...
%directory = '/data1/user_data/bacteria/imageoverlaybuffer/';   %uigetdir; %directory containing the images you want to analyze
...
p{7,1}= 'Wiener';                p{7,2}=2;          % 1 = enable Wiener2 adaptive denaoise filter, 0 = disable
...
for PIVresult=1:size(x,1)
    disp('RUNNING PIV_POSTPROCESSING');
    disp(int2str(PIVresult));
...
clearvars -except filepath filebase_image filebase_figresults filebase_aviresults filebase_results csvresults name p s x y u v typevector directory Files u_filt v_filt typevector_filt k name
disp('CMND LN DONE.')
